  ...
  return (
    <div className="mx-auto w-full max-w-content px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mt-5 space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-0 flex-1 basis-64">
            ...
            <input
              ...
              placeholder="Search prompts, descriptions, tags, content…"
              aria-label="Search prompts"
              className="h-10 sm:h-9 w-full rounded-md border border-border bg-input pl-9 pr-8 text-base sm:text-sm text-foreground placeholder:text-muted-foreground/70 transition-colors focus:border-ring/70 focus:outline-none focus:ring-2 focus:ring-ring/30"
            />
            {search && (
              <button
                type="button"
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <button
                ...
                className={cn(
                  'flex h-[32px] sm:h-[30px] w-9 sm:w-8 items-center justify-center rounded-[5px] transition-colors',
        ...
        {/* Quick horizontal tag filter strip */}
        {tagData && tagData.tags.length > 0 && (
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1 -mx-4 px-4 sm:mx-0 sm:px-0">
            ...
            {tagData.tags.map((t) => {
              const isSelected = view === 'tag' && params.tagId === t.id
              return (
                <button
                  key={t.id}
                  type="button"
                  onClick={() => navigate(`/app/tags/${t.id}`)}
                  className={cn(
                    'shrink-0 rounded-full px-3 py-1 text-xs font-medium transition-colors',
                    isSelected
                      ? 'bg-primary text-primary-foreground font-semibold shadow-sm'
